...
def interaction_2():
    if request.method == 'POST':
        ...
        suggestions_style = request.form.get('suggestionsRadioOptions')
        # print(f"the story style selcted is {story_style}")

        # (caption, story) = runModels_langchain(upld_path,story_style)
        (caption, suggestions) = runModels_langchain_RAG(upld_path,suggestions_style,db_dir)

        

        return render_template('milestone2.html', active='interaction_2', imgPath=img_path, suggestions=suggestions, caption=caption, style=suggestions_style)
    else:
